...
class TFL:
    ...
    def init_EM(self, data, prev_frame_id, curr_frame_id) -> None:
    ...
    def run_product(self, image_path, index):
        ...
        '''part1'''
        candidate, auxiliary = self.detect_of_source_lights(image_path)
    ...
    def dist(self, current_frame):
        ...
        print(current_frame.traffic_light)
    ...
    def is_tfl(self, candidate, auxiliary, image_path) -> Tuple[list, list]:
        traffic = []
        auxiliary_traffic = []
        ...
        for i, coord in enumerate(candidate):
            crop_images = self.get_crop_images2(image_pad, coord)
    ...
    def detect_of_source_lights(self, image_path: Any):
    ...
    def source_light(self, image_path, candidate, auxiliary):
        image = np.array(Image.open(image_path))
        x, y = 0, 1
    ...
    def get_crop_images(self, image_path, candidate) -> List:
        image = np.array(Image.open(image_path))
        crop_images = []
        for i, coord in enumerate(candidate):
    ...
    def get_crop_images2(self, image, coord):
        crop_image = image[coord[1]:coord[1] + 81, coord[0]:coord[0] + 81]
        return crop_image
